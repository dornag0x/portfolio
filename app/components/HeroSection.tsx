	...
	return (
		<div className="relative min-h-screen flex items-center justify-center overflow-hidden">
			...
			<div className="absolute inset-0 overflow-hidden">
				{[
					{ icon: '🦀​', delay: '0s', position: 'top-20 left-1/4' },
			...
			<div className="relative z-10 max-w-5xl mx-auto px-4">
				<div className="text-center space-y-6">
					...
					<div className="space-y-4 sm:space-y-6 mt-6 sm:mt-8">
						<div className="flex flex-col items-center gap-4">
							<div className="flex items-center gap-3">
								...
								<h2 className="text-xl sm:text-2xl font-light tracking-wide text-blue-400">Entrepreneur / Solana Developer</h2>
								<span className="h-[1px] w-8 sm:w-12 bg-blue-500"></span>
							</div>
							<p className="text-base sm:text-lg text-gray-400 max-w-2xl px-4 sm:px-0">
								Turning bold ideas into fast, on-chain products on Solana.
							</p>
						</div>

						{/* Tech Stack Pills */}
						<div className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-6 sm:mt-8 px-4 sm:px-0">
							{['Rust', 'TypeScript', 'Node.js', 'React', 'GraphQL'].map((tech) => (
								<span
									key={tech}
					...
					<div className="flex justify-center gap-4 sm:gap-6 mt-8 sm:mt-12">
						{[
							{ name: 'GitHub', icon: '🐙', href: 'https://github.com/dornag0x'},
							{ name: 'LinkedIn', icon: '💼', href: 'https://www.linkedin.com/in/hugo-feufeu-97608233a/' },
							{ name: 'Twitter', icon: '🐦', href: 'https://x.com/dornag0x' },
						].map((social) => (
							<a key={social.name} target='blank' href={social.href} className="group relative p-2 sm:p-3 hover:text-blue-400 transition-colors" aria-label={social.name}>
